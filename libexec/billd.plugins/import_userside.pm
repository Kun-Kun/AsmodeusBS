...
sub insert_user {
  ...
  if (!$user->{errno}) {
    ...
    $Log->log_print('LOG_USER_ADD', $user_info->{USERS}->{LOGIN}, "UID:$uid", { LOG_FILE => "/usr/abills/var/log/import_userside_log" });
  }
}
...
sub change_user {
  ...
  # }
  $Log->log_print('LOG_USER_CHANGE', $user_old_info->{LOGIN}, "UID:$user_old_info->{UID} $change_log", { LOG_FILE => "/usr/abills/var/log/import_userside.log" });
